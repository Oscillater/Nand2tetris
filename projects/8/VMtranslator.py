...
class VMTranslator:
    ...
    def writebootstrap(self):
        with open(self.output_file,'a') as outputfile:
            line1='@256\nD=A\n@SP\nM=D\n'
            number=5
            return_address=str('Sys.init')+'returnaddress'
            line2='@'+return_address+'\nD=A\n@SP\nA=M\nM=D\n@LCL\nD=M\n@SP\nM=M+1\nA=M\nM=D\n@ARG\nD=M\n@SP\nM=M+1\nA=M\nM=D\n@THIS\nD=M\n@SP\nM=M+1\nA=M\nM=D\n@THAT\nD=M\n@SP\nM=M+1\nA=M\nM=D\n@SP\nMD=M+1\n@ARG\nM=D\n@'+str(number)+'\nD=A\n@ARG\nM=M-D\n@SP\nD=M\n@LCL\nM=D\n'
            line3='@Sys.init\n0;JMP\n('+str(return_address)+')\n'
        ...
        with open(self.input_file,'r') as input_file:
            ...
            while self.parser.hasMoreCommands():
                ...
                if commandtype==CommandType.C_PUSH or commandtype==CommandType.C_POP:
                    ...
                    self.codewriter.label=self.parser.arg1()
                    self.codewriter.number=int(self.parser.arg2())
...
class CodeWriter():
    ...
    def writeCall(self):
        with open(self.output_file,'a') as outputfile:
            ...
            return_address=str(self.label)+'returnaddress'+str(CodeWriter.time)
            CodeWriter.time=CodeWriter.time+1
            line='@'+return_address+'\nD=A\n@SP\nA=M\nM=D\n@LCL\nD=M\n@SP\nM=M+1\nA=M\nM=D\n@ARG\nD=M\n@SP\nM=M+1\nA=M\nM=D\n@THIS\nD=M\n@SP\nM=M+1\nA=M\nM=D\n@THAT\nD=M\n@SP\nM=M+1\nA=M\nM=D\n@SP\nMD=M+1\n@ARG\nM=D\n@'+str(number)+'\nD=A\n@ARG\nM=M-D\n@SP\nD=M\n@LCL\nM=D\n'
            outputfile.write(line)
        self.writeGoto()
